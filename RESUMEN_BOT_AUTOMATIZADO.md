# Bot Docker Automatizado para upeer

## 🎯 Objetivo Cumplido

Automatizar un contenedor Docker que solicite contacto a una instancia de upeer y responda automáticamente a mensajes.

## 🤖 Especificaciones del Bot

### Información Técnica

- **Nombre del contenedor:** `upeer-bot-user`
- **Imagen Docker:** `upeer-bot` (construida desde `Dockerfile.peer`)
- **Red:** Yggdrasil con 3 peers públicos
- **Puerto:** 50005 (UDP)

### Identidad del Bot (actual)

- **upeer ID:** `808bcb86a1ec73fd71353abebe3a09af`
...
- **Proof of Work:** Implementa `generate_light_proof()` compatible con upeer
...
2. **Usuario:** Acepta solicitud manualmente en UI de upeer
...
```bash
docker logs -f upeer-bot-user
```

### Detener Bot

```bash
docker rm -f upeer-bot-user
```
...
1. Usuario aceptó solicitud en UI upeer
...
```bash
docker rm -f upeer-bot-user
docker run --name upeer-bot-user --cap-add=NET_ADMIN --device=/dev/net/tun -v /tmp/revelnest_shared:/shared -e NODE_ENV_NAME="user_bot" -e TARGET_IDENTITY="a169346e249181b306156c7caa265d6d:7704:49e5:b4cd:7910:2191:2574:351b" upeer-bot
```
...
1. **Aceptar solicitud** en tu UI de upeer
...
- Logs completos de upeer (últimos 2 minutos)
- Captura de pantalla de la solicitud de contacto
- Output de `docker logs upeer-bot-user --tail 50`

---

_Última actualización: 5 Marzo 2026 - Bot configurado y en ejecución_
