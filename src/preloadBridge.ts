...
const eventApi = {
    ...
    onKeyChangeAlert: (callback: (data: { upeerId: string; oldFingerprint: string; newFingerprint: string; alias: string }) => void) => subscribe<[{ upeerId: string; oldFingerprint: string; newFingerprint: string; alias: string }]>('key-change-alert', (data) => callback(data)),
    onMessageDelivered: (callback: (data: { id: string; upeerId: string }) => void) => subscribe<[{ id: string; upeerId: string }]>('message-delivered', (data) => callback(data)),
    onMessageRead: (callback: (data: { id: string; upeerId: string }) => void) => subscribe<[{ id: string; upeerId: string }]>('message-read', (data) => callback(data)),
    ...
    onChatCleared: (callback: (data: { upeerId: string }) => void) => subscribe<[{ upeerId: string }]>('chat-cleared', (data) => callback(data)),
    onMessageStatusUpdated: (callback: (data: { id: string; status: string }) => void) => subscribe<[{ id: string; status: string }]>('message-status-updated', (data) => callback(data)),
    onTyping: (callback: (data: { upeerId: string }) => void) => subscribe<[{ upeerId: string }]>('peer-typing', (data) => callback(data)),
    onFocusConversation: (callback: (data: { upeerId?: string; groupId?: string }) => void) => subscribe<[{ upeerId?: string; groupId?: string }]>('focus-conversation', (data) => callback(data)),
