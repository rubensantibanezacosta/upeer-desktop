...
export const SecurityModal: React.FC<SecurityModalProps> = ({
    ...
    // Simple deterministic derivation for a "Safety Number"
    // In a real app, this would be a hash of sorted public keys
    const deriveSafetyNumber = (pk1: string, pk2: string) => {
        const sorted = [pk1, pk2].sort().join('');
        // Just a mock for now, ideally we'd use a real hash function here if available in renderer
        // or pass it from main process. Let's just use the hex for now.
        return sorted.slice(0, 40).match(/.{1,5}/g)?.join(' ') || sorted;
    };

    const safetyNumber = deriveSafetyNumber(myPublicKey, contactPublicKey);
            ...
            <ModalDialog sx={{ width: 500, maxWidth: '95vw', borderRadius: 'xl', boxShadow: 'lg', p: 0, overflow: 'hidden' }}>
                ...
                <DialogContent sx={{ p: 4, textAlign: 'center' }}>
                    <Stack spacing={3} alignItems="center">
                        ...
                        >
                            RevelNest no almacena tus llaves privadas. Todo el proceso de cifrado ocurre localmente en tu dispositivo.
                        </Alert>
