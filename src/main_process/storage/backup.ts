...
/**
 * Verifica si ya existe un backup de hoy en el directorio.
 */
function hasBackupToday(userDataPath: string): boolean {
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    const backupPattern = new RegExp(`^p2p-chat\\.db\\.backup-${today}$`);
    
    try {
        const files = fs.readdirSync(userDataPath);
        return files.some(file => backupPattern.test(file));
    } catch (err) {
        error('Failed to check for today\'s backup', err, 'backup');
        return false;
    }
}
 ...
 */
export function performDatabaseBackup(userDataPath: string, maxBackupsDays: number = 7): string | undefined {
 ...
 */
export function scheduleBackups(userDataPath: string, intervalHours: number = 24): () => void {
