...
export function initIdentity(userDataPath: string) {
    ...
    if (!fs.existsSync(flagPath) && fs.existsSync(legacyKeyPath)) {
        try { fs.renameSync(legacyKeyPath, legacyKeyPath + '.legacy-bak'); } catch (_) { }
...
export function decryptSealed(ephemeralPublicKey: Buffer, nonce: Buffer, ciphertext: Buffer): Buffer | null {
    if (_isLocked) return null;
    const plaintext = Buffer.alloc(ciphertext.length - sodium.crypto_box_MACBYTES);
    const ok = sodium.crypto_box_seal_open(plaintext, ciphertext, publicKey, secretKey);
    return ok ? plaintext : null;
}
