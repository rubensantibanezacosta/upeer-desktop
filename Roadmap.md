...
- [x] **Rendimiento del recovery de archivos grandes desde el vault** — el recovery de vídeos/multimedia grandes se atascaba o iba muy lento. Causas y fixes: (1) `handleVaultDelivery` disparaba `tryRecoverVaultTransferByFileHash` **por cada shard** → escaneos O(N²) en DB; ahora se acumulan los `fileHash` del batch y el recovery se dispara **una sola vez por archivo** al final del batch. (2) El **ACK borraba los shards del custodio antes de completar la paginación**, haciendo el `offset` inestable y dejando el archivo a medias; ahora el ACK se retrasa hasta la última página (`hasMore: false`). (3) `VAULT_DELIVERY_PAGE_SIZE`/`MAX_DELIVERY_ENTRIES` subidos de 50 a 120 y `validateVaultDelivery` a 120, reduciendo los round-trips (58→24 para un vídeo de 30MB). Añadido test e2e multiproceso de **archivo grande (~2MB) reconstruido por segmentos sin atascarse** (192 shards, 2 páginas). 19 tests multiproceso en verde.
