...
function getContactByAddress(address) {
  return db.select().from(contacts).where(eq(contacts.address, address)).get();
}
...
function getMessageStatus(id) {
  const msg = db.select({ status: messages.status }).from(messages).where(eq(messages.id, id)).get();
  return msg ? msg.status : null;
}
function getContacts() {
  const contactsList = db.select().from(contacts).all();
  const result = contactsList.map((c) => {
    const lastMsgObj = db.select().from(messages).where(eq(messages.chatRevelnestId, c.revelnestId || "")).orderBy(desc(messages.timestamp)).limit(1).get();
    return {
      ...c,
...
function deleteContact(revelnestId2) {
  return db.delete(contacts).where(eq(contacts.revelnestId, revelnestId2)).run();
}
function updateLastSeen(revelnestId2) {
  return db.update(contacts).set({ lastSeen: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(contacts.revelnestId, revelnestId2)).run();
}
function closeDB() {
