...
// Helper to generate valid 32-char hex upeer IDs
...
describe('Kademlia DHT', () => {
    ...
    it('should convert upeer ID to Kademlia ID', () => {
        const upeerId = 'test1234567890abcdef1234567890abcd';
        const kademliaId = toKademliaId(upeerId);
    ...
    it('should add and find contacts', () => {
        const contactUpeerId = hexId('contact');
        const contact = {
        ...
        // Find contact
        const found = dht.findContact(contact.upeerId);
        assert.ok(found);
        assert.strictEqual(found.upeerId, contact.upeerId);
    ...
    it('should find closest contacts', () => {
        ...
        for (let i = 0; i < 10; i++) {
            const contactUpeerId = hexId('node' + i);
            const contact = {
                nodeId: toKademliaId(contactUpeerId),
                upeerId: contactUpeerId,
                address: `200:1:2:3::${i}`,
        ...
        console.log('Closest contacts:', closest.map((c: any) => c.upeerId));
    });

    it('should handle store and find value', async () => {
        const keyUpeerId = hexId('testkey');
        const key = toKademliaId(keyUpeerId);
    ...
    it('should perform maintenance without errors', () => {
        // Add some contacts
        for (let i = 0; i < 5; i++) {
            const contactUpeerId = hexId('maint' + i);
            const contact = {
                nodeId: toKademliaId(contactUpeerId),
                upeerId: contactUpeerId,
