# upeer Kademlia DHT Protocol

## Overview

The Kademlia Distributed Hash Table (DHT) is a decentralized peer-to-peer routing and storage system that enables efficient discovery and data retrieval in the upeer network. This document describes the implementation details, message formats, and protocols used by upeer's Kademlia DHT.

## Key Design Decisions

### 1. Node Identifiers

- **upeer ID**: 32-character hex string (128 bits) derived from Ed25519 public key
- **Kademlia ID**: 160-bit SHA-256 hash of upeer ID (first 20 bytes)
...
interface KademliaContact {
  ...
  upeerId: string; // Original upeer ID (32-char hex)
...
All DHT messages are wrapped in the standard upeer packet format with cryptographic signatures.
...
**Key Derivation**: `SHA-256("location:" + upeerId).slice(0, 20)`
...
1. DNS TXT records: `_dht-seeds._udp.upeer.chat`
2. HTTPS endpoint: `https://api.upeer.chat/v1/seeds`
...
await kademlia.storeLocationBlock(upeerId, {
  address: newIp,
...
_Last Updated: Implementation Phase 12 - Kademlia DHT Refactoring_
