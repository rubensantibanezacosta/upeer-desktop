...
1. **Hook `useFileTransfer`** - Gestión de estado
   - Comunicación con API `window.revelnest`
...
### Errores Corregidos
1. **TypeScript declarations** - Interface `Window.revelnest` actualizada
...
git clone https://github.com/revelnest/chat-p2p

# 2. Instalar dependencias
npm install

# 3. Iniciar aplicación (base de datos limpia)
rm -rf ~/.config/RevelNest\ Chat/p2p-chat.db
npm start

# 4. Abrir segunda instancia para pruebas
# En otra terminal:
npm start -- --user-data-dir=~/.config/RevelNest-chat-test
```

### Para desarrolladores:
- **API disponible**: `window.revelnest` con métodos de transferencia
...
1. **Thumbnails en Electron**: Algunas rutas no generan preview
   - *Workaround*: Funcionalidad principal funciona sin thumbnails

2. **Eventos de progreso**: Latencia variable en actualizaciones
   - *Workaround*: Barra se actualiza con cada ACK de fragmento

3. **Base de datos vieja**: Error "no such column: renewal_token"
   - *Solución*: Eliminar `~/.config/RevelNest Chat/p2p-chat.db`
...
**RevelNest ahora soporta transferencia segura de archivos** además de mensajería E2EE, estableciendo las bases para funcionalidades multimedia avanzadas en fases futuras.

---
**Documentación adicional:**
- [Guía de pruebas manuales](./skills/gu-a-de-pruebas-manuales-para-transferencia-de-archivos-revelnest-mmdjar2b)
- [Componentes UI](./skills/componentes-ui-para-transferencia-de-archivos-en-revelnest-mmdgzc54)
- [Guía de integración](./skills/gu-a-de-integraci-n-de-transferencia-de-archivos-en-revelnest-mmdgtgij)
